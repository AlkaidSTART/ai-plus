...
## 一、拟使用的模型与 AI 能力
- **基础大模型**：采用 Claude 4 系列 Active 模型（示例：`claude-sonnet-4-6`）作为主力推理引擎，承担多语言评论归一、痛点结构化聚类与双栏改款建议生成；复杂约束下的商业博弈与战略决策选用同期 Active 高推理能力模型；使用 Claude Vision 进行买家实拍图（色差、破损、工艺瑕疵）的多模态视觉取证（仅辅助线索，不默认精确物理归因；低质量图片可能误判，坐标为近似值）。原文档中的 `claude-3-7-sonnet-20250219`（已于 2026-02-19 退役，请求会失败）与 `claude-3-opus-20240229`（已于 2026-01-05 退役）不再使用；模型须可配置并记录替换日期、回归样本与单任务成本（R03）。
...
#### 1. 模型分层调用与路由策略
在 `backend/core/` 中统一封装模型调用客户端，根据任务算力与推理深度需求实施分层路由：
- **快速提取与归一**：Active Sonnet 模型（温度值 `0.2`），负责结构化字段提取与多语言翻译标准化。
- **多模态视觉取证**：Active Sonnet 模型（视觉模式），单次最多挂载 5 张买家实拍图，提取视觉缺陷标签（P1 能力；输出区分可见现象/模型推断/待工程确认，毫米级改动须人工确认）。
- **复杂战略与财务否决**：同期 Active 高推理能力模型（开启思考模式/高推理预算），结合成本公式执行现金流风险推演。
...
async def call_claude_vision(image_bytes: bytes, mime_type: str, prompt: str) -> dict:
    ...
    response = await anthropic_client.messages.create(
        model="claude-sonnet-4-6",  # 示例 Active 模型，以目标账号可调用性与回归样本确定；退役 ID（claude-3-7/3-opus 系列）已不可用
        max_tokens=2048,
        temperature=0.1,
...
- **运行模式**：采用 HuggingFace `sentence-transformers` 或 `FlagEmbedding` 在 FastAPI 容器内加载或独立部署为轻量 Embedding 微服务。P0 明确实施 Dense 语义表示与独立聚类逻辑；Sparse/BM25 融合待检索评测证明必要后再加入。真实模式缺语义模型依赖时失败或显示结构化降级原因，禁止用哈希 Fake Embedding 伪装真实语义分析（R08）。
...
#### 3. 成本控制与 Token 优化
- **Prompt Caching**：对静态的跨境品类质检规范、行业成本基准表等长系统提示词启用 Anthropic Prompt Caching（缓存前缀）。“固定降低 80% 成本”改为待测目标：实际收益取决于复用前缀与命中情况，按选定模型的当期计费和实际 usage 计算。
- **结构化输出约束**：优先采用官方 JSON Schema 结构化输出 + 严格工具输入替代仅靠提示词/正则抽取 JSON 的主路径；仍须处理拒绝、输出截断和本地业务校验（字段合规不代表事实正确）。

---

## 二、AI Agent 与工作流设计
- **Agent 编排架构**：基于 LangGraph 1.2 构建多状态环路协同架构（须配置 checkpointer 方可按 thread 保存/恢复图状态；数据库任务列表 ≠ 可恢复的图执行状态）。队列（Celery worker）负责投递/并发/重试，checkpointer 负责图恢复，数据库负责业务结果；外部调用不保证天然只执行一次，核心结果写入须在任务完成前确认成功。解耦数据采集、多模态清洗、跨语种语义聚类、双栏改款决策（本体 vs 包装履约）、成本与现金流否决、全链路证据回溯与后验回测 7 大核心阶段。
...
from agents.nodes.data_node import fetch_reviews_node
from agents.nodes.vision_node import vision_audit_node
from agents.nodes.cluster_node import semantic_cluster_node
from agents.nodes.decision_node import dual_track_decision_node
from agents.nodes.finance_node import financial_veto_node
from agents.nodes.evidence_node import evidence_traceback_node
from agents.nodes.backtest_node import backtest_eval_node
...
## 三、数据处理方式
- **多源数据采集**：原始评论、Amazon 聚合主题（Customer Feedback API，仅聚合洞察/每周更新/英语，非批量原文导出）、Keepa（商品背景与价格历史，`reviews` 为评分数时序、`images` 为商品轮播图，不能当作评论正文/买家图来源）分开接入并保存来源与覆盖范围；商业评论服务（如 Rainforest 系）仅为试采候选，须记录接口版本、字段契约、分页上限、留存条件与费用后再定。
...
```python
# backend/crawler/amazon_fetcher.py
import httpx
...
系统采用**前后端分离架构**：前端为独立部署的 Vue 3 单页应用（SPA），后端为统一的 Python FastAPI 服务（REST API + SSE 流式推送 + 鉴权），前后端仅通过 HTTP 协议通信，无 BFF 中间层。

- **前端架构**：基于 `Vue 3.5`（Composition API + `<script setup>`）+ `Vite 8` + `TypeScript` 构建高响应式单页中台；使用 `Vue Router` 承担页面路由、`Pinia` 承担全局状态管理；集成 `TailwindCSS 4` 打造现代化看板；集成 `ECharts 6` 呈现价格弹性曲面、多维差评词云雷达与 ROI 敏感度矩阵；通过原生 `EventSource` / `fetch` 流式读取消费后端 SSE 长连接。
- **后端架构**：采用 Python `FastAPI 0.141` 作为唯一后端服务，统一承担 REST 业务接口（Pydantic 请求/响应模型校验）、JWT 会话鉴权、CORS 跨域配置与 SSE 实时事件流下发；借助 `Celery 5.6 + Redis 8` 实现重型爬虫任务与 LangGraph 1.2 复杂多轮 Agent 图状态异步调度；通过 `uvicorn` 多 Worker 部署。
- **数据库与存储**：选用 `PostgreSQL 18 + pgvector 0.8` 作为统一的结构化与向量存储底座；选用 `Redis 8` 承载任务发布订阅（Pub/Sub）、中间状态缓存与分布式锁；选用 `MinIO / AWS S3` 存储抓取的买家实拍高清图。
- **基础设施与部署**：采用前后端分离的双目录仓库（`frontend/` + `backend/`），前端依赖由 `bun` 管理、后端依赖由 `uv` 管理；本地使用 `Docker Compose` 一键编排所有依赖容器；生产环境前端以静态产物部署于 `Vercel / CDN + Nginx`，后端 FastAPI 服务部署于 `Railway / AWS ECS`。
...
```text
ai-plus/
├── frontend/                  # Vue 3 + Vite 前端应用（独立部署）
│   ├── src/
│   │   ├── api/               # Axios/Fetch 请求封装与 SSE 客户端
│   │   ├── views/             # 路由页面（大盘/竞品/VOC/决策台/风控）
│   │   ├── components/
│   │   │   ├── charts/         # ECharts 封装组件
...
│   │   └── main.ts
│   ├── index.html
│   ├── vite.config.ts         # 含 /api 代理配置（开发环境转发至 FastAPI）
│   └── package.json           # bun 管理依赖
├── backend/                   # Python FastAPI 后端（统一 API + AI 核心）
...
#### 2. 前后端通信协议规范（SSE 流式直连）
- **选择依据**：改款 Agent 与视觉多模态分析属于长耗时任务（10s–45s），采用 **Server-Sent Events (SSE)** 实现服务端向前端实时下发节点推进状态（如 `1. 正在检索差评...` $\rightarrow$ `2. 正在质检实拍图...` $\rightarrow$ `3. 触发财务否决引擎...` $\rightarrow$ `4. 完成报告输出`）。前后端分离架构下，Vue 前端通过 `EventSource` 直接与 FastAPI 建立 SSE 长连接，无需 BFF 转发层；后端通过 `CORSMiddleware` 放行前端来源域名。
- **FastAPI SSE 端点与 CORS 配置实现**：
...
# 前后端分离跨域放行：生产环境通过 ALLOWED_ORIGINS 环境变量收敛来源
app.add_middleware(
    CORSMiddleware,
    allow_origins=["http://localhost:5173"],  # Vite 开发服务器默认地址
    allow_credentials=True,
    allow_methods=["*"],
    allow_headers=["*"],
)

app.include_router(insight_router, prefix="/api/v1")  # 版本前缀只挂一次（R07：示例中 router 内 prefix 与 include prefix 重复挂载须修正）
```
...
export function subscribeTaskEvents(taskId: string, onEvent: (data: TaskEvent) => void) {
  const base = import.meta.env.VITE_API_BASE_URL ?? "";
  const es = new EventSource(`${base}/api/v1/insight/tasks/${taskId}/events`);

  es.addEventListener("message", (e) => {
    const payload = JSON.parse((e as MessageEvent).data);
    onEvent(payload);
    if (payload.step === "COMPLETED") es.close();
  });
...
# [Service Routing]
VITE_API_BASE_URL=http://localhost:8000   # FastAPI 后端地址（Vite 构建时静态替换；运行静态容器时设环境变量不会更新已构建 JS，须选构建时注入或运行时配置方案）
BACKEND_CORS_ORIGINS=http://localhost:5173 # FastAPI 放行的前端来源
```
...
services:
  ...
  backend:
    ...
    container_name: insightx-backend
    environment:
  ...
  frontend:
    build:
      context: ./frontend
      dockerfile: Dockerfile
    container_name: insightx-frontend
    environment:
